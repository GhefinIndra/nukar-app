...
// Callback handler untuk ORDER (bukan topup)
exports.handleOrderCallback = async (req, res) => {
  try {
    const callbackData = req.body;
    
    console.log('📩 Received ORDER callback from Finpay:', JSON.stringify(callbackData, null, 2));
    ...
    // Get order by Finpay order ID
    const Order = require('../models/Order');
    const order = await Order.findByFinpayOrderId(parsed.orderId);
    ...
    if (parsed.paymentStatus === 'CAPTURED' || parsed.paymentStatus === 'PAID') {
      ...
      await Order.updatePaymentStatus(order.order_number, 'paid');
      
      console.log(`✅ Order payment success! Order: ${order.order_number}, Amount: ${order.total_amount}`);
      ...
      // Payment failed
      await Order.updatePaymentStatus(order.order_number, 'failed');
      
      console.log(`❌ Order payment failed! Order: ${order.order_number}`);
    ...
    console.error('Order callback handler error:', error);
    return errorResponse(res, 'Callback processing failed', 500, error.message);
  }
};